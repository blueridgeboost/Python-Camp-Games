...
def draw():
    ...
    if not game_over:
        ...
        if show_countdown:
            screen.draw.text(str(count), color="black",topleft=(CENTER_X - 65,150), fontsize=60)
    else:
        screen.clear()
        screen.blit("stage",(0,0))
        screen.draw.text("Score: "+ str(score), color="black",topleft=(10,10))
        screen.draw.text("GAME OVER!"+ str(score), color="black",topleft=(CENTER_X-130,220))
    return
